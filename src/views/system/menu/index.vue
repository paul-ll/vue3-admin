          ...
          >
            <template #default="{ node, data }">
              <span class="custom-tree-node">
                ...
                <span>
                  <a @click.stop="handleCreateChildMenu(data)">
                    添加
                  </a>
                  <a @click.stop="handleRemoveMenu(node, data)">
                    删除
                  </a>
                </span>
              </span>
...
import { addNewMenu, removeMenuByID } from '@/api/menu'
...
export default defineComponent({
  ...
  setup() {
    ...
    // 重新刷新整个系统
    const reloadPage = () => {
      proxy?.$confirm('菜单已发生改动，是否要刷新当前系统', '刷新确认', {
        type: 'warning'
      }).then(() => {
    ...
    watch(dialogVisible, value => { // 关闭面板 重置表单
      if (!value) {
